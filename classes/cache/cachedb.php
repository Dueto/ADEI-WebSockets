...
#	echo "Time: $time -> $sqltime\n";
 ...
 function GetCachePoints($postfix = false, MASK &$mask = NULL, INTERVAL &$ivl = NULL, $type = CACHEDB::TYPE_AUTO, $limit = 0, $amount_or_sampling = 0, $resolution = false) {
  ...
  function GetDownloads($download = NULL, $sort = NULL) {
    ...
    if(!$download) {
	$res = @mysql_query("SELECT * FROM downloads ORDER BY startdate $sort", $this->dbh);   
    }
    else{     
      $res = @mysql_query("SELECT * FROM downloads WHERE dl_id='".$download."' ORDER BY startdate $sort;", $this->dbh);
    }
    if(!$res) throw new ADEIException(translate("Can't get downloads from database, error") . ": " . mysql_error($this->dbh));
    else return $res;
  }
